...
##############################################################################################################
enrich_inWDRS= all_together2%>%
                dplyr::filter(isin_WDRS== "TRUE")
enrich_add_stat_inWDRS= data.frame(table(enrich_inWDRS$enrich_add_stat))%>%
                             dplyr::mutate(pct= paste0(round(prop.table(Freq), 4)*100, "%"),
                                           Freq= Freq/1000)%>%
                             dplyr::filter(!Var1== "")

enrich_add_stat_inWDRS2= data.frame(table(enrich_inWDRS$enrich_add_stat))%>%
  dplyr::mutate(pct= paste0(round(prop.table(Freq), 4)*100, "%"),
                Freq= Freq)%>%
  dplyr::filter(!Var1== "")

enrich_add_tt_inWDRS= ggplot(enrich_add_stat_inWDRS, aes(Var1, Freq, fill= Var1)) + geom_bar(stat= "identity") +
  geom_text(aes(label= pct), vjust= -0.5, size= 3.0) +
  xlab(label= "Enrich Address Category") + ylab("Counts('000)") +
  ggtitle("Negative Labs Data(is_inWDRS== TRUE) - Address Enrichment") +
